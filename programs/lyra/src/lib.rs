...
pub mod lyra {
    ...
    pub fn play_game(context: Context<PlayGame>, game_id: u64, request_id: u64) -> Result<()> {
        msg!("{:?}, {:?}", game_id, request_id);
        let config = &context.accounts.config;
        let game = &context.accounts.game;
        let game_data = &context.accounts.game_data;
        let current_timestamp = Clock::get()?.unix_timestamp as u64;
        let player_balance = context.accounts.player.to_account_info().lamports();
